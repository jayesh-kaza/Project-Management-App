...
function SideBar({ open, handleDrawerClose, routes }) {
  ...
  const list1 = {
    Favorites: [null, routes[4].path],
    "All Items": [null, routes[5].path],
    "Deleted Items": [null, routes[6].path]
  };
  const list2 = {
    "Tasks I've Created": [null, routes[7].path],
    "Assigned to Others": [null, routes[8].path],
    "Recent Tasks": [null, routes[9].path]
  };
  const items = {
    Home: [
    ...
    "My Projects": [
      <GroupWorkIcon fontSize="small" className={classes.icon} />,
      routes[2].path
    ],
    Teams: [
      <GroupRoundedIcon fontSize="small" className={classes.icon} />,
      routes[3].path
    ]
  };
  ...
  return (
    ...
    >
      ...
      <Typography
        style={{ paddingLeft: "15px", color: "#9ca6af", fontSize: "12px" }}
        variant="body2"
      >
        Favorite by clicking {star}{" "}
      </Typography>
